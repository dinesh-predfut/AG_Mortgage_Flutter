...
//  String formattedDate = DateFormat('yyyy-MM-dd').format(selectedDay);
  ...
  Future<List<SeletArea>> fetchAreasByCity(int cityId) async {
    try {
  ...
  int convertToNumber(dynamic value) {
    if (value is int) {
      return value; // If it's already an int, return it directly
    } else if (value is String) {
      return int.tryParse(value) ?? 0; // Try parsing, return 0 if it fails
    } else {
      return 0; // Return default value for unsupported types
    }
  }
  ...
  String formatProfileDate(String profileDate) {
    // Parse the profileDate string to DateTime
